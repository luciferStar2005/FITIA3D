...
import type { PlanHoyData, ProgresoRutinaData } from '../types';
...
export default function Home() {
    ...
    const [fechasCompletadas, setFechasCompletadas] = useState<string[]>([]);
    const [fechasParciales, setFechasParciales] = useState<string[]>([]);
    ...
    const tileContent = ({ date, view }: { date: Date; view: string }) => {
        if (view === 'month') {
            ...
            const fechaCalendario = format(date, 'yyyy-MM-dd');

            if (fechasCompletadas.includes(fechaCalendario)) {
                const color = fechasParciales.includes(fechaCalendario) ? 'bg-yellow-400' : 'bg-green-500';
                return <div className={`w-1.5 h-1.5 ${color} rounded-full mx-auto mt-0.5`} />;
            }
        ...
        const fetchFechasConRutina = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    setFechasCompletadas(data.fechasCompletadas || []);
                    setFechasParciales(data.fechasParciales || []);
    ...
    useEffect(() => {
        const fetchRacha = async () => {
            try {
                ...
                const response = await fetch('http://localhost:8080/api/v1/rutinas/racha', {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    useEffect(() => {
        const fetchProgresoPorFecha = async () => {
            try {
                if (!token) return;
                const fechaFormateada = format(fecha, 'yyyy-MM-dd');
                const response = await fetch(
                    `http://localhost:8080/api/v1/rutinas/progreso?fecha=${fechaFormateada}`,
                    {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                ...
                if (response.ok) {
                    const data: ProgresoRutinaData = await response.json();
                    setCompletados(data.ejerciciosCompletadosIds || []);
                    setRachaActual(data.rachaActual || 0);
                }
            } catch (error) {
    ...
    const finalizarRutina = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:8080/api/v1/rutinas/finalizar', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                console.error("Error al finalizar rutina:", await response.text());
                alert("No se pudo guardar el progreso de la rutina.");
                return;
            }

            const data: ProgresoRutinaData = await response.json();
            setRachaActual(data.rachaActual || 0);
            setCompletados(data.ejerciciosCompletadosIds || completados);

            if (data.cumplioMeta) {
                setFechasCompletadas(prev => prev.includes(fechaFormateada) ? prev : [...prev, fechaFormateada]);
                setFechasParciales(prev => {
                    const sinFecha = prev.filter(x => x !== fechaFormateada);
                    return data.diaPerfecto ? sinFecha : [...sinFecha, fechaFormateada];
                });
            } else {
                setFechasCompletadas(prev => prev.filter(x => x !== fechaFormateada));
                setFechasParciales(prev => prev.filter(x => x !== fechaFormateada));
            }

            const mensaje = data.cumplioMeta
                ? `🔥 ¡Excelente trabajo!\n\nCompletaste el ${data.porcentaje}% de tu rutina.\nRacha actual: ${data.rachaActual} días.`
                : `😅 Completaste el ${data.porcentaje}% de la rutina.\n\nNecesitas al menos 60% para mantener la racha.`;
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-5 shadow-lg">
                            ...
                            <div className="mt-3 flex items-center gap-2 px-3 py-2 bg-neutral-800/50 border border-neutral-800 rounded-xl text-xs text-gray-400">
                                ...
                                {fechasCompletadas.includes(format(fecha, 'yyyy-MM-dd')) && (
